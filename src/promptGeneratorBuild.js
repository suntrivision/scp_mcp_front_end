  ...
  const outputReporting =
    p.outputFormat === 'table'
      ...
      : p.outputFormat === 'deep'
        ...
        : 'Return JSON only with keys: intent, summary, kpis (object), narrative (string: main explanatory paragraph), recommendations (array of short actionable strings), rows (array of objects). No markdown fences.';

  const outputException = `Return JSON only (no markdown) with keys: intent, summary, kpis (counts by category), narrative (main explanatory paragraph), recommendations (array of concise planner actions), rows (array of objects with item, location, root_cause_category, exception_type, severity High|Medium|Low, recommended_action, detail).`;
